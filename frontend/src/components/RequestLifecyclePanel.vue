  ...
  <div class="lifecycle">
    ...
    <template v-if="data">
      ...
      <section>
        ...
        <p v-if="openConflicts.length" class="alert">{{ t('lifecycle.conflicts.blocking') }}</p>

        <ul v-if="data.document_conflicts.length" class="rows">
          <li v-for="row in data.document_conflicts" :key="row.id">
            ...
            <template v-if="row.resolved_at">
              <p class="row-detail">{{ t('lifecycle.conflicts.fields.authority_consulted') }}: {{ row.authority_consulted }}</p>
              <p class="row-detail">{{ t('lifecycle.conflicts.fields.authoritative_document') }}: {{ row.authoritative_document }}</p>
              <p class="row-detail">{{ t('lifecycle.conflicts.fields.correction_note') }}: {{ row.correction_note }}</p>
            </template>
            <template v-else>
              <p class="row-open">{{ t('lifecycle.conflicts.stillOpen') }}</p>
              <button
        ...
        <form v-else class="form" @submit.prevent="submitConflict">
          <label>
          ...
          <div class="actions">
            ...
            <button type="button" @click="openForm = ''">{{ t('lifecycle.cancel') }}</button>
          </div>
        </form>
        ...
        <form v-if="openForm === 'resolveConflict'" class="form" @submit.prevent="submitResolve">
      ...
      <section v-if="data.document_validity.length">
        ...
        <ul class="rows">
          <li v-for="row in data.document_validity" :key="row.attachment_id">
            ...
            <button v-can="'meeting_outputs.edit'" class="compact" type="button" @click="startValidity(row)">
              {{ row.card ? t('lifecycle.validity.recheckAction') : t('lifecycle.validity.action') }}
            </button>
          </li>
        </ul>

        <form v-if="openForm === 'validity'" class="form" @submit.prevent="submitValidity">
      ...
      <section>
        ...
        <ul v-if="data.special_cases.length" class="rows">
          <li v-for="row in data.special_cases" :key="row.id">
            <p class="row-head">
              <strong>{{ row.kind_label }}</strong>
              <span v-if="row.recorded_at">{{ row.recorded_at.slice(0, 10) }}</span>
            </p>
            <p v-for="(value, key) in row.determinations" :key="key" class="row-detail">
              {{ t(`lifecycle.specialCases.fields.${key}`) }}: {{ value }}
            </p>
            <p v-if="row.halt_progress && !row.resolved_at" class="row-open">{{ t('lifecycle.specialCases.halted') }}</p>
            <template v-if="row.resolved_at">
              <p class="row-detail">{{ row.resolution_note }}</p>
            </template>
            <button
              v-else
              v-can="'meeting_outputs.edit'"
        ...
        <form v-else class="form" @submit.prevent="submitSpecialCase">
          <label>
      ...
      <section>
        ...
        <ul v-if="data.corrections.length" class="rows">
          <li v-for="row in data.corrections" :key="row.id">
            <p class="row-head">
              <strong>{{ row.kind_label }}</strong>
              <span v-if="row.approved_at">{{ t('lifecycle.corrections.approved') }}</span>
              <span v-else class="row-open">{{ t('lifecycle.corrections.pending') }}</span>
            </p>
            <p class="row-detail">{{ row.detail }}</p>
            <p class="row-detail">{{ row.incorrect_value }} ← {{ row.corrected_value }}</p>
            <button
        ...
        <form v-else class="form" @submit.prevent="submitCorrection">
      ...
      <section>
        ...
        <ul v-if="data.withdrawals.length" class="rows">
          <li v-for="row in data.withdrawals" :key="row.id">
            ...
            <p class="row-detail">{{ row.reason }}</p>
            <p v-if="row.determination_note" class="row-detail">{{ row.determination_note }}</p>
            <button
        ...
        <template v-if="isRequester && !data.has_open_withdrawal">
          <button v-if="openForm !== 'withdrawal'" class="compact" type="button" @click="openForm = 'withdrawal'">
            {{ t('lifecycle.withdrawals.action') }}
          </button>

          <form v-else class="form" @submit.prevent="submitWithdrawal">
            <label>
        ...
        <form v-if="openForm === 'determination'" class="form" @submit.prevent="submitDetermination">
          <label>
