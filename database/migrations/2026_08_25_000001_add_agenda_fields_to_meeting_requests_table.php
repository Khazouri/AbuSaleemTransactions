...
/**
 * Stage 31 — the agenda outgrows "an ordered list of requests": an item
 ...
 * request to borrow those from.
 *
 * `request_id` moves to nullable via `->change()` (a separate
 ...
 * key and the `(meeting_id, request_id)` unique index are preserved;
 * MySQL and SQLite both treat every NULL in a unique index as distinct, so
 * several admin items with no request can coexist on the same meeting.
 */
return new class extends Migration
{
    public function up(): void
    {
        Schema::table('meeting_requests', function (Blueprint $table) {
            $table->string('item_type')->default('employee_request')->after('request_id');
